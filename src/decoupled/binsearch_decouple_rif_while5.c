...
void kernel(
    ...
    for (uint32_t i = 0; i < table_elements;) {
        ...
        if (rif < RIF && table_i < table_elements) {
            ...
            if(equal || !(l<=r)){
                hls_stream_enq_uint32t(index_stream2, index);
                hls_stream_enq_uint32t(result_stream, res);
                i++;
                rif--;
                continue;
            }
    ...
    for (uint32_t i = 0; i < table_elements; i++) {
        uint32_t index = hls_stream_deq_uint32t(index_stream2, OUTSTANDING_READS);
        uint32_t res = hls_stream_deq_uint32t(result_stream, OUTSTANDING_READS);
        result[index] = res;
    }
}
